...
async def send_welcome_email(to_email: str, name: str) -> bool:
    ...
    subject = "Ваша бесплатная проба готова — Dicto"
    content = (
        _mono("Добро пожаловать в Dicto")
        + _h(f"{greeting}.")
        + _p(
            "Вы зарегистрировались в Dicto — сервисе расшифровки и AI-разбора записанных разговоров. "
            "На счёте <strong>30 бесплатных минут и первый AI-разбор</strong> — попробуйте на своей "
            "записи без карты и подписки."
        )
        + _p(
        ...
        + (
            ...
            '<li>Загрузить запись разговора, консультации или интервью</li>'
            '<li>Получить текст с таймкодами и разметкой спикеров</li>'
            '<li>Получить AI-разбор: саммари, тезисы и задачи — бесплатно на первой записи</li>'
            '<li>Нужно больше разборов и чат по записи — оформить Pro или пополнить кошелёк</li>'
            '</ul>'
        )
    ...
    text = (
        f"{greeting}.\n\n"
        "Вы зарегистрировались в Dicto — на счёте 30 бесплатных минут и первый AI-разбор.\n\n"
        "Что сделать прямо сейчас:\n"
        "- Загрузить запись (MP3, WAV, MP4, OGG и ещё 8 форматов)\n"
        "- Получить текст с таймкодами и спикерами\n"
        "- Получить AI-разбор: саммари, тезисы и задачи (бесплатно на первой записи)\n"
        "- Больше разборов и чат по записи — на Pro или с кошелька\n\n"
        f"Начать: {settings.APP_URL}/upload\n\n"
        "Поддержка: support@dicto.pro"
    )
    return await send_email(to_email, subject, _wrap_html(content, "Бесплатная проба: 30 минут + AI-разбор"), text)
