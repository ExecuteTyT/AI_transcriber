    ...
    with SyncSession() as db:
        ...
        try:
            ...
            ydl_opts = {
                ...
                "socket_timeout": 60,
                # ВАЖНО: НЕ добавлять tv-клиент в player_client. Из-за эксперимента
                # YouTube (issue #12563) tv-клиент ложно помечает живые видео как
                # "DRM protected" и роняет всю экстракцию, ВЫБРАСЫВАЯ рабочие форматы
                # default-клиента (проверено 2026-06-23: KVlN8b1T_mg default=3/3 OK,
                # а default,tv=ложный DRM). Дефолтный набор клиентов yt-dlp качает
                # обычные видео сам; настоящий DRM (редко) → «недоступно» — приемлемо.
            ...
            # Duration-decision (зеркало upload, общая plan_by_duration): файл
            # длиннее баланса → не блокируем, а обрежем до баланса в ffmpeg перед
            # Voxtral. Длительность из yt-dlp metadata (download дёшев — экономим Voxtral).
            ...
            transcription.full_duration_sec = int(info.get("duration") or 0)
            if decision["action"] == "block":
                transcription.status = "failed"
                transcription.error_message = decision["paywall"]["message"]
                db.commit()
                logger.info("URL transcription %s blocked: no balance", transcription_id)
                return
            if decision["action"] == "truncate":
                # Скачали целиком (bandwidth дёшев), но Voxtral запустим только на
                # первых max_minutes — ffmpeg обрежет в process_transcription.
                transcription.max_minutes = decision["max_minutes"]
                logger.info(
                    "URL transcription %s truncated to %d of %d min",
                    transcription_id, decision["max_minutes"], decision["full_minutes"],
                )
...
def _translate_ytdlp_error(msg: str) -> str:
    ...
    # DRM (лицензионный контент: фильмы/сериалы/платное) — потоки зашифрованы,
    # скачать НЕВОЗМОЖНО ничем (ни cookies, ни прокси, ни PO-token). На web-клиенте
    # это маскируется под «not available», но tv-клиент отдаёт явный «DRM protected»
    # (проверено 2026-06-22 на OFfUoVzcKgc). Говорим честно, чтобы юзер не считал
    # это нашим сбоем и не ретраил.
    if "drm" in m:
        return (
            "Это видео защищено DRM (фильм, сериал или платный контент) — "
            "скачать и расшифровать его технически невозможно. Попробуйте другое видео."
        )
    # «Video unavailable» / «This video is not available» — yt-dlp выдаёт это для
    # удалённых/региональных видео И иногда для DRM/доступных видео (web-клиент не
    # вытащил форматы). Нормальные видео с нашего IP качаются (проверено 2026-06-22),
    # так что НЕ утверждаем «удалено» — честно зовём попробовать RU-источник.
...
def _is_permanent_ytdlp_error(msg: str) -> bool:
    ...
    permanent_markers = (
        ...
        "drm",  # DRM-защита — нескачиваемо в принципе, ретрай бессмыслен
