  ...
  if (!user?.is_admin) {
    return (
      <div className="text-center py-24">
        <h2 className="text-xl font-bold text-[var(--fg)] mb-2">Доступ запрещён</h2>
        <p className="text-[var(--fg-subtle)] mb-6">У вас нет прав администратора</p>
    ...
    <div className="animate-fade-up">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-[var(--fg)]">Админ-панель</h1>
        </div>
        <p className="text-sm text-[var(--fg-subtle)]">Управление сервисом Dicto</p>
      </div>
        ...
        {([["overview", "Обзор"], ["users", "Пользователи"], ["transcriptions", "Транскрипции"]] as const).map(([key, label]) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
              tab === key ? "bg-[var(--bg-elevated)] text-[var(--fg)] shadow-sm" : "text-[var(--fg-subtle)] hover:text-[var(--fg-muted)]"
            }`}
          >
            {label}
          </button>
      ...
      {tab === "overview" && stats && (
        <div className="space-y-6 animate-fade-up">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {[
              ...
              <div key={stat.label} className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border)] p-4 md:p-5">
                ...
                <p className="text-2xl md:text-3xl font-bold text-[var(--fg)] tabular-nums">{stat.value.toLocaleString()}</p>
                {stat.sub && <p className="text-xs text-[var(--fg-subtle)] mt-1">{stat.sub}</p>}
              </div>
            ))}
          </div>
          ...
          {/* Status distribution */}
          <div className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border)] p-5 md:p-6">
            <h3 className="text-sm font-semibold text-[var(--fg)] mb-4">Статусы транскрипций</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {Object.entries(stats.transcriptions_by_status).map(([status, count]) => (
                <div key={status} className="text-center p-3 rounded-xl bg-[var(--bg-muted)]">
                  <p className="text-2xl font-bold text-[var(--fg)] tabular-nums">{count}</p>
      ...
      {loading && tab === "overview" && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border)] p-5 animate-pulse">
              <div className="h-3 bg-[var(--bg-muted)] rounded w-1/2 mb-3" />
      ...
      {tab === "users" && (
        <div className="animate-fade-up">
          ...
          <p className="text-xs text-[var(--fg-subtle)] mb-4">Всего: {usersTotal}</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border)] bg-[var(--bg-muted)]">
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">План</th>
                    <th className="text-left py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">Использование</th>
                    <th className="text-left py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">Записей</th>
                    <th className="text-right py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">Действия</th>
                  </tr>
                </thead>
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-muted)] transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <select
                          ...
                          className="text-xs border border-[var(--border-strong)] rounded-lg px-2 py-1 bg-[var(--bg-elevated)] text-[var(--fg)]"
                        >
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            className="p-1.5 rounded-lg text-[var(--fg-subtle)] hover:text-primary-600 hover:bg-primary-50 transition-colors text-xs"
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg text-[var(--fg-subtle)] hover:text-red-500 hover:bg-red-50 transition-colors"
                            title="Удалить"
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {tab === "transcriptions" && (
        <div className="animate-fade-up">
          <p className="text-xs text-[var(--fg-subtle)] mb-4">Всего: {transTotal}</p>
          <div className="bg-[var(--bg-elevated)] rounded-2xl border border-[var(--border)] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border)] bg-[var(--bg-muted)]">
                    <th className="text-left py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">Файл</th>
                    <th className="text-left py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">Пользователь</th>
                    <th className="text-left py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">Статус</th>
                    <th className="text-left py-3 px-4 font-semibold text-[var(--fg-subtle)] text-xs uppercase tracking-wider">Длительность</th>
                ...
                <tbody>
                  {transcriptions.map((t) => (
                    <tr key={t.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-muted)] transition-colors">
                      <td className="py-3 px-4">
                        <p className="font-medium text-[var(--fg)] truncate max-w-[200px]">{t.title}</p>
                      </td>
                      <td className="py-3 px-4 text-xs text-[var(--fg-subtle)]">{t.user_email}</td>
                      <td className="py-3 px-4">
                        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-md ${statusColors[t.status] || ""}`}>{t.status}</span>
                      </td>
                      <td className="py-3 px-4 text-[var(--fg-muted)] tabular-nums">
